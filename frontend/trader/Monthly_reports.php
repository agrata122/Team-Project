...
try {
    ...
    $monthlyStatsQuery = "SELECT 
        ...
        AND o.order_date >= ADD_MONTHS(TRUNC(SYSDATE), -12)
        GROUP BY TRUNC(o.order_date, 'MM')
        ORDER BY TRUNC(o.order_date, 'MM') DESC";

    $stmt = oci_parse($conn, $monthlyStatsQuery);
    ...
    $monthlyPaymentQuery = "SELECT 
        TO_CHAR(TRUNC(p.payment_date, 'MM'), 'YYYY-MM-DD') as month_start,
        ...
        ORDER BY TRUNC(p.payment_date, 'MM') DESC";

    $stmt = oci_parse($conn, $monthlyPaymentQuery);
    oci_bind_by_name($stmt, ":user_id", $user_id);
    oci_execute($stmt);
...
<head>
    ...
    <title>Monthly Reports - FresGrub</title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li>
                <a href="Monthly_reports.php" class="active">
    ...
    <div class="main-content">
        ...
        <div class="metrics">
            <div class="metric-card">
                <div class="metric-label">Total Orders (Last 12 Months)</div>
                <div class="metric-value"><?php echo array_sum(array_column($monthlyStats, 'TOTAL_ORDERS')); ?></div>
            </div>
            <div class="metric-card">
                <div class="metric-label">Total Sales (Last 12 Months)</div>
                <div class="metric-value">$<?php echo number_format(array_sum(array_column($monthlyStats, 'TOTAL_SALES')), 2); ?></div>
            </div>
            <div class="metric-card">
                <div class="metric-label">Completed Orders</div>
                <div class="metric-value"><?php echo array_sum(array_column($monthlyStats, 'COMPLETED_ORDERS')); ?></div>
            </div>
            <div class="metric-card">
                <div class="metric-label">Pending Orders</div>
                <div class="metric-value"><?php echo array_sum(array_column($monthlyStats, 'PENDING_ORDERS')); ?></div>
            </div>
        </div>

        <!-- Monthly Orders Chart -->
        <div class="card">
            <h3 class="card-title">Monthly Orders Overview</h3>
        ...
        <div class="card">
            <h3 class="card-title">Monthly Orders Details</h3>
            <table>
                <thead>
                    <tr>
                        <th>Month</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td><?php echo date('F Y', strtotime($stat['MONTH_START'])); ?></td>
        ...
        <div class="card">
            <h3 class="card-title">Monthly Payments Details</h3>
            <table>
                <thead>
                    <tr>
                        <th>Month</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td><?php echo date('F Y', strtotime($payment['MONTH_START'])); ?></td>
    ...
    <script>
        ...
        const labels = monthlyStats.map(stat => {
            const date = new Date(stat.MONTH_START);
        ...
        new Chart(document.getElementById('monthlyOrdersChart'), {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Total Orders',
                    data: monthlyStats.map(stat => stat.TOTAL_ORDERS),
                    backgroundColor: '#2e7d32',
                }, {
                    label: 'Completed Orders',
                    data: monthlyStats.map(stat => stat.COMPLETED_ORDERS),
        ...
        new Chart(document.getElementById('monthlySalesChart'), {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Total Sales',
                    data: monthlyStats.map(stat => stat.TOTAL_SALES),
                    backgroundColor: '#0288d1',
                }, {
                    label: 'Completed Sales',
                    data: monthlyStats.map(stat => stat.COMPLETED_SALES),
